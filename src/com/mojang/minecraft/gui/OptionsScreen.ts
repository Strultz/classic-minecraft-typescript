...
export class OptionsScreen extends Screen {
    ...
    public override init2(): void {
        for (let i: number = 0; i < this.options.numOptions; ++i) {
            this.buttons.push(new SmallButton(i, Math.trunc(this.width / 2) - 155 + i % 2 * 160, Math.trunc(this.height / 6) + 24 * (i >> 1), this.options.getMessage(i)))
        }
    ...
    public override buttonClicked(button: Button): void {
        if (button.active) {
            if (button.id < 100) {
                ...
                button.message = this.options.getMessage(button.id)
            }
            if (button.id == 100) {
